// frontend/app/dashboard/create-profile/page.js
'use client';
...
export default function CreateProfilePage() {
  ...
  const steps = [
    { id: 1, title: 'Learning Style', icon: '🎯' },
    { id: 2, title: 'Subject & Level', icon: '📚' },
    { id: 3, title: 'Focus Areas', icon: '🎪' }
  ];
  ...
  const handleNext = () => {
    if (validateStep(currentStep)) {
      // If moving to step 3, generate focus areas automatically if none exist
      if (currentStep === 2 && customFocusAreas.length === 0 && formData.subject.trim()) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submissionData = {
        ...
        name: user?.name || 'User' // Use Google auth name automatically
      };
             ...
             <div className="flex space-x-3">
               {currentStep < 3 ? (
                 <Button
                   ...
                   disabled={currentStep === 2 && !formData.subject.trim()}
                 >
                   Continue
                 </Button>
               ) : (
